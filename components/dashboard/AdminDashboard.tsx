...
const StatCard = ({ title, value, currencySymbol = '' }: { title: string, value: string | number, currencySymbol?: string }) => (
    <Card>
    ...
    <div className="space-y-3">
        {items.slice(0, 3).map(item => (
            <div key={item.id} className="p-3 border rounded-md flex justify-between items-center bg-white hover:bg-gray-50">
                <div>
                    <p className="font-semibold">{ (typeof item[titleKey] === 'object' && item[titleKey] !== null) ? item[titleKey][language] : item[titleKey] }</p>
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <Card className="bg-base-100">
                    ...
                    <QuickAccessList items={pendingProjects} titleKey="title" subtitleKey="location" type="project" />
                </Card>
                 <Card className="bg-base-100">
                            ...
                            <tbody>
                                {activeProjects.slice(0, 3).map(p => (
                                    <tr key={p.id} className="bg-white border-b">
                                        <td className="px-6 py-4 font-semibold">{p.title[language]}</td>
